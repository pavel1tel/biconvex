                      ...
                      <Table.Td>
                        <div className={classes.tableFavoriteCellWrapper}>
                          <Favorite
                            ...
                            defaultValue={
                              (localStorage.getItem("fav_coins") != null ? JSON.parse(localStorage.getItem("fav_coins")!) : []).filter(
                                (e) => e.pair === pair.substring(0, row.symbol.length) + "/" + pair.substring(row.symbol.length),
                              ).length > 1
                            }
                            pair={pair.substring(0, row.symbol.length) + "/" + pair.substring(row.symbol.length)}
                          />
                  ...
                  <Table.Td>
                    <div className={classes.tableFavoriteCellWrapper}>
                      <Favorite
                        ...
                        defaultValue={
                          (localStorage.getItem("fav_coins") != null ? JSON.parse(localStorage.getItem("fav_coins")!) : []).filter(
                            (e) => e.pair == row.symbol + "/USDT",
                          ).length > 0
                        }
                        pair={row.symbol + "/USDT"}
                      />
                      <p>{row.symbol + "/USDT"}</p>
                    </div>
                  </Table.Td>
            ...
            ? (localStorage.getItem("fav_coins") != null ? JSON.parse(localStorage.getItem("fav_coins")!) : [])
                ...
                .map((coin) => (
                  <Table.Tr
                    ...
                    className={clsx(classes.tableRow, classes.noSelect, {
                      [classes.rowHover]: true,
                      [classes.rowSelected]: selectedRow === coin.pair,
                    })}
                  >
                    <Table.Td>
                      <div className={classes.tableFavoriteCellWrapper}>
