...
function useForm({
  ...
  const setFieldValue = React.useCallback((path, payload) => {
    ...
    _setValues((current) => {
      const currentValue = getPath.getPath(path, current);
      const result = setPath.setPath(
        path,
  ...
  const isDirty = (path) => {
    ...
    return !isEqual__default.default(values, valuesSnapshot.current);
  };
